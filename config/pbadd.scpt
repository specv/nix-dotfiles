-- https://apple.stackexchange.com/questions/339201/how-do-i-copy-multiple-files-to-macoss-clipboard-programmatically
...
on run input
    if input's class = script then set input to Finder's selection as alias list
...
to addToClipboard(fs)
    ...
    repeat with f in fs
        ...
        if (FileManager's fileExistsAtPath:fp) then (fURLs's addObject:(NSURL's fileURLWithPath:fp))
    end repeat
